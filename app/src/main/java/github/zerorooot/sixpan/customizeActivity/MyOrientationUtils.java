...
public class MyOrientationUtils {
    ...
    private final GSYBaseVideoPlayer mVideoPlayer;
    ...
    private final boolean mIsAutoLandscapeAndReverseLandscape;

    public MyOrientationUtils(Activity activity, GSYBaseVideoPlayer gsyVideoPlayer,boolean autoLandscapeAndReverseLandscape) {
//        super(activity, gsyVideoPlayer);
        this.mActivity = new WeakReference(activity);
        this.mVideoPlayer = gsyVideoPlayer;
        this.mOrientationOption = new OrientationOption();
        this.mIsAutoLandscapeAndReverseLandscape = autoLandscapeAndReverseLandscape;
    ...
    protected void init() {
        ...
        if (mIsAutoLandscapeAndReverseLandscape) {
            mOrientationEventListener = getAutoLandscapeOrientationEventListener(context);
        } else {
            mOrientationEventListener = getWithSystemOrientationEventListener(context);
        }

        mOrientationEventListener.enable();
    }

    private void setLandHandle() {
        if (mClick) {
    ...
    private void setLandscapeHandle() {
        if (mClick) {
    ...
    private void setReverseLandscapeHandle() {
        if (mClick) {
    ...
    private OrientationEventListener getWithSystemOrientationEventListener(Context context) {
    ...
    private OrientationEventListener getAutoLandscapeOrientationEventListener(Context context) {
        return new OrientationEventListener(context) {
            @SuppressLint("SourceLockedOrientationActivity")
            @Override
            public void onOrientationChanged(int rotation) {
                ...
                // 设置横屏
                if (((rotation >= mOrientationOption.getNormalLandAngleStart())
                        && (rotation <= mOrientationOption.getNormalLandAngleEnd()))) {
                    setLandscapeHandle();
                    Log.d("msj", "横屏");
                }
     ...
     */
    @SuppressLint("SourceLockedOrientationActivity")
    public void resolveByClick(boolean isClick) {
